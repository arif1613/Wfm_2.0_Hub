...
{
    class when_receiving_a_change_request_from_a_SuperAdmin
    {
        ...
        private static ICommonIdentity _identity;
                ...
                Post["/{holder_id}/testResource/{id}", true] = async (parameters, token) =>
                {
                    using (var session = _bootstrapper.DocumentStore.OpenAsyncSession())
                    {
                        ...
                        var result = this.RequiresAuthorization<TestResource>(session, (Guid)parameters.holder_id,
                                                                              (Guid)parameters.id, new []{_role});

                        _identity = Thread.CurrentPrincipal.Identity as ICommonIdentity;
        ...
        private Establish context = () =>
        {
            ...
            {
                session.StoreAsync(new UserWithHolder
                {
                    Id = Guid.NewGuid().AsId(typeof(UserWithHolder)),
                    Username = _userName,
                ...
                session.StoreAsync(new UserWithHolder
                {
                    ...
                    Username = _userName,
                    Roles = new List<string> { UserRoles.SuperAdmin },
                    Password = _password
                }).Wait();
        ...
        private It the_identity_of_the_thread_should_be_set = () => _identity.ShouldNotBeNull();
        private It the_user_id_of_the_identity_should_be_set = () => _identity.Id.ShouldEqual(_userId);
        private It the_owner_of_the_user_should_be_set = () => _identity.OwnerId.ShouldEqual(_holderId);
    }
}
