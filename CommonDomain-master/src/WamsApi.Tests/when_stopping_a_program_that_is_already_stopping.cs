...
{
    public class when_stopping_a_program_that_is_already_stopping : TestBase
    {
        ...
        private Establish context = () =>
        {
            ...
            _program.Start();
            _task = _program.StopAsync();
        };

        private Because of = () =>
            {
                MediaApi.StopProgram(_program.Id, ConnectionString).Await();
                _program = _channel.Programs.Where(p => p.Id == _program.Id).First();
            };

        private It the_method_should_return_when_the_program_has_finished_stopping =
            () => _program.State.ShouldEqual(ProgramState.Stopped);
